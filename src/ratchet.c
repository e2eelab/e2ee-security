...
static int create_chain_key(
    ...
    if (is_valid_cipher_suite(cipher_suite)) {
        if ((our_private_key == NULL && ratchet_public_key == NULL) ||
            (our_private_key != NULL && ratchet_public_key != NULL)) {
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        cipher_suite->hf_suite->hkdf(
            ...
            root_key.data,
            root_key.len,
            (uint8_t *)KDF_INFO_RATCHET,
            sizeof(KDF_INFO_RATCHET) - 1,
            derived_secrets,
            sizeof(derived_secrets));
        ...
        if (new_chain_key->shared_key.data) {
            overwrite_protobuf_from_array(
                &(new_chain_key->shared_key), derived_secrets + shared_key_len);
        } else {
            copy_protobuf_from_array(
                &(new_chain_key->shared_key), derived_secrets + shared_key_len, shared_key_len);
        }
...
static int advance_chain_key(const cipher_suite_t *cipher_suite, E2ees__ChainKey *chain_key) {
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        cipher_suite->hf_suite->hmac(
            chain_key->shared_key.data,
            chain_key->shared_key.len,
            CHAIN_KEY_SEED,
    ...
    if (is_valid_cipher_suite(cipher_suite)) {
        ...
        if (ret == E2EES_RESULT_SUCC) {
            if (payload->sequence < chain->index) {
                e2ees_notify_log(
                    NULL, BAD_MESSAGE_SEQUENCE, "verify_and_decrypt_for_existing_chain()");
                ret = E2EES_RESULT_FAIL;
            }
        }
    } else {
        e2ees_notify_log(
            NULL, BAD_CIPHER_SUITE, "verify_and_decrypt_for_existing_chain() invalid cipher suite");
        ret = E2EES_RESULT_FAIL;
    }
        ...
        if (ret == E2EES_RESULT_SUCC) {
            ...
            if (create_msg_keys(cipher_suite, new_chain, &mk) == E2EES_RESULT_SUCC) {
                ret = verify_and_decrypt(
                    decrypted_data_out, decrypted_data_len_out, cipher_suite, ad, mk, payload);
...
static size_t verify_and_decrypt_for_new_chain(
    ...
    if (is_valid_cipher_suite(cipher_suite)) {
        ...
        if (is_valid_one2one_msg_payload(payload)) {
            ...
            if (pqc_param) {
                if (payload->ratchet_key.len !=
                    cipher_suite->kem_suite->get_param().kem_ciphertext_len) {
                    ret = E2EES_RESULT_FAIL;
                }
            } else {
                if (payload->ratchet_key.len !=
                    cipher_suite->kem_suite->get_param().asym_pub_key_len) {
                    ret = E2EES_RESULT_FAIL;
                }
            }
        } else {
        ...
        e2ees_notify_log(
            NULL, BAD_CIPHER_SUITE, "verify_and_decrypt_for_new_chain() invalid cipher suite");
        ret = E2EES_RESULT_FAIL;
    }
...
static void prune_bloated_skipped_keys(E2ees__Ratchet *ratchet) {
    ...
    // Free the oldest keys at the front of the array, indices 0 to
    // (excess_count - 1)
    size_t i;
    ...
    E2ees__SkippedMsgKeyNode **new_list = (E2ees__SkippedMsgKeyNode **)malloc(
        sizeof(E2ees__SkippedMsgKeyNode *) * MAX_SKIPPED_MESSAGE_KEY_NODES);
    ...
    free_mem(
        (void **)&(ratchet->skipped_msg_key_list),
        sizeof(E2ees__SkippedMsgKeyNode *) * ratchet->n_skipped_msg_key_list);
...
int initialise_as_bob(
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        ret = cipher_suite->hf_suite->hkdf(
            ...
            salt,
            sizeof(salt),
            (uint8_t *)KDF_INFO_ROOT,
            sizeof(KDF_INFO_ROOT) - 1,
            derived_secrets,
            derived_secrets_len);
    }
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        ratchet->receiver_chain =
            (E2ees__ReceiverChainNode *)malloc(sizeof(E2ees__ReceiverChainNode));
        ...
        copy_protobuf_from_protobuf(
            &(receiver_chain->our_ratchet_private_key), &(our_ratchet_key->private_key));
        ...
        copy_protobuf_from_array(
            &(sender_chain->chain_key->shared_key),
            derived_secrets + shared_key_len,
            shared_key_len);
        ...
        if (cipher_suite->kem_suite->get_param().pqc_param == false) {
            // ECC mode
            copy_protobuf_from_protobuf(
                &(sender_chain->our_ratchet_public_key), &(our_ratchet_key->public_key));
        } else {
            // PQC mode
...
int initialise_as_alice(
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        copy_protobuf_from_array(
            &(receiver_chain->chain_key->shared_key),
            derived_secrets + shared_key_len,
            shared_key_len);
        ...
        if (pqc_param == false) {
            // ECC mode
            copy_protobuf_from_protobuf(
                &(receiver_chain->their_ratchet_public_key), their_ratchet_key);
        } else {
            // PQC mode
            copy_protobuf_from_protobuf(
                &(receiver_chain->their_ratchet_public_key), their_encaps_ciphertext);
        }
        ...
        if (pqc_param == false) {
            ...
            copy_protobuf_from_protobuf(
                &(sender_chain->our_ratchet_public_key), &(our_ratchet_key->public_key));
            copy_protobuf_from_protobuf(
                &(sender_chain->their_ratchet_public_key), their_ratchet_key);
        } else {
            // PQC mode
            create_chain_key(
                cipher_suite,
                ...
                &(ratchet->root_key),
                sender_chain->chain_key,
                &(sender_chain->our_ratchet_public_key));
            copy_protobuf_from_protobuf(
                &(sender_chain->their_ratchet_public_key), their_ratchet_key);
        }
...
int encrypt_ratchet(
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        *payload_out      = (E2ees__One2oneMsgPayload *)malloc(sizeof(E2ees__One2oneMsgPayload));
        ...
        copy_protobuf_from_protobuf(
            &((*payload_out)->ratchet_key), &(sender_chain->our_ratchet_public_key));
...
int decrypt_ratchet(
    ...
    *decrypted_data_len_out                                = 0;
                ...
                if (coming_root_sequence < our_root_sequence) {
                    if (coming_root_sequence == our_root_sequence - 1) {
                        if (compare_protobuf(
                            ...
                            if (corresponding_receiver_chain->chain_key->index >
                                payload->sequence) {
                    ...
                    /**
                     * If their root sequence is bigger than ours, then only one
                     * condition can be satisfied: coming_root_sequence =
                     * our_root_sequence + 1
                     */
                    if (coming_root_sequence != our_root_sequence + 1) {
                        e2ees_notify_log(
                            ...
                            "decrypt_ratchet() coming_root_sequence: %d, "
                            "our_root_sequence: %d",
                            coming_root_sequence,
                            our_root_sequence);
                        ret = E2EES_RESULT_FAIL;
                    }
                }
    ...
    if (ret == E2EES_RESULT_SUCC) {
        if (skipped_message == true) {
            ...
            for (i = 0; i < ratchet->n_skipped_msg_key_list; i++) {
                if (payload->sequence == ratchet->skipped_msg_key_list[i]->msg_key->index &&
                    ...
                    ret = verify_and_decrypt(
                        ...
                        cipher_suite,
                        ad,
                        ratchet->skipped_msg_key_list[i]->msg_key,
                        payload);

                    if (ret == E2EES_RESULT_SUCC) {
                        // decryption success, so we need to eliminate the
                        // skipped message
                        if (ratchet->n_skipped_msg_key_list == 1) {
                            /*
                             * Edge case: If there is only 1 skipped message key
                             * left in the list, and we have just successfully
                             * used it for decryption, the list should now be
                             * empty. We free the memory and explicitly set the
                             * pointer to NULL. This prevents the dangerous
                             * malloc(0) behavior in the else-block and ensures
                             * it passes the strict is_valid_ratchet()
                             * validation.
                             */
                            ...
                            /*
                             * Normal case: There are multiple skipped message
                             * keys. We simply free the used key, shift the
                             * remaining array elements left, and optionally
                             * shrink the array to save memory.
                             */
                            e2ees__skipped_msg_key_node__free_unpacked(
                                ratchet->skipped_msg_key_list[i], NULL);

                            if (i < ratchet->n_skipped_msg_key_list - 1) {
                                memmove(
                                    &(ratchet->skipped_msg_key_list[i]),
                                    &(ratchet->skipped_msg_key_list[i + 1]),
                                    sizeof(E2ees__SkippedMsgKeyNode *) *
                                        (ratchet->n_skipped_msg_key_list - i - 1));
                            }

                            (ratchet->n_skipped_msg_key_list)--;
                            E2ees__SkippedMsgKeyNode **temp_skipped_message_keys =
                                (E2ees__SkippedMsgKeyNode **)realloc(
                            ...
                            if (temp_skipped_message_keys != NULL) {
                                ...
                                e2ees_notify_log(
                                    NULL,
                                    DEBUG_LOG,
                                    "decrypt_ratchet(): Memory allocation "
                                    "failed for skipped_msg_key_list.");
                                ret = E2EES_RESULT_FAIL;
                            }
                        }
                        break;
                        ...
                        e2ees_notify_log(
                            ...
                            "verify_and_decrypt() in decrypt_ratchet()");
                    }
                }
            }
            if (ret != E2EES_RESULT_SUCC) {
                // the corresponding message key not found or the decryption
                // failed
            ...
            if (corresponding_receiver_chain == NULL) {
                 ...
                 * We will store our new chain key later when decrypting the
                 * message correctly. */
                ret = verify_and_decrypt_for_new_chain(
                    decrypted_data_out, decrypted_data_len_out, cipher_suite, ratchet, ad, payload);
                if (ret != E2EES_RESULT_SUCC) {
                    // the decryption failed
                    e2ees_notify_log(
                        NULL,
                        BAD_MESSAGE_DECRYPTION,
                        "verify_and_decrypt_for_new_chain() in "
                        "decrypt_ratchet()");
                }
            } else {
                /* They use the same ratchet key. The sequence of the
                 * payload(incoming message) may be bigger than or equal to the
                 * index of our receiver chain. */
                ret = verify_and_decrypt_for_existing_chain(
                    decrypted_data_out,
                    decrypted_data_len_out,
                    cipher_suite,
                    ad,
                    corresponding_receiver_chain->chain_key,
                    payload);
                if (ret != E2EES_RESULT_SUCC) {
                    // the decryption failed
                    e2ees_notify_log(
                        NULL,
                        BAD_MESSAGE_DECRYPTION,
                        "verify_and_decrypt_for_existing_chain() in "
                        "decrypt_ratchet()");
                }
            }
        }
    ...
    if (ret == E2EES_RESULT_SUCC) {
        // if the decryption is success, we will update our ratchet state if
        // neccessary
        if (corresponding_receiver_chain == NULL) {
            if (skipped_message == false) {
                ...
                if (payload->sending_message_sequence - ratchet->received_message_sequence >
                    ...
                    if (ratchet->n_skipped_msg_key_list + skipped_num >
                        ...
                        if (ratchet->skipped_msg_key_list == NULL) {
                            ratchet->skipped_msg_key_list = (E2ees__SkippedMsgKeyNode **)malloc(
                                sizeof(E2ees__SkippedMsgKeyNode *) * skipped_num);
                            if (ratchet->skipped_msg_key_list == NULL) {
                                e2ees_notify_log(
                                    NULL,
                                    DEBUG_LOG,
                        ...
                        if (ret == E2EES_RESULT_SUCC) {
                            ...
                            for (cur_seq = 0; cur_seq < skipped_num; cur_seq++) {
                                // insert data
                                E2ees__SkippedMsgKeyNode *key = (E2ees__SkippedMsgKeyNode *)malloc(
                                    sizeof(E2ees__SkippedMsgKeyNode));
                                ...
                                if (create_msg_keys(
                                        cipher_suite,
                                        ratchet->receiver_chain->chain_key,
                                        &(key->msg_key)) != E2EES_RESULT_SUCC) {
                                    e2ees__skipped_msg_key_node__free_unpacked(key, NULL);
                                    ret = E2EES_RESULT_FAIL;
                                    break;
                                }
                                copy_protobuf_from_protobuf(
                                    &(key->ratchet_key_public),
                                    &(ratchet->receiver_chain->their_ratchet_public_key));
                                ...
                                if (advance_chain_key(
                                        cipher_suite, ratchet->receiver_chain->chain_key) !=
                                    E2EES_RESULT_SUCC) {
                                    ret = E2EES_RESULT_FAIL;
                                    break;
                                }
                ...
                if (ret == E2EES_RESULT_SUCC) {
                    ...
                    E2ees__ReceiverChainNode *new_receiver_chain =
                        (E2ees__ReceiverChainNode *)malloc(sizeof(E2ees__ReceiverChainNode));
                    e2ees__receiver_chain_node__init(new_receiver_chain);

                    // copy their ratchet key
                    copy_protobuf_from_protobuf(
                        &(new_receiver_chain->their_ratchet_public_key), &(payload->ratchet_key));

                    // create a new receiver chain key
                    new_receiver_chain->chain_key =
                        (E2ees__ChainKey *)malloc(sizeof(E2ees__ChainKey));
                    ...
                    E2ees__SenderChainNode *new_sender_chain =
                        (E2ees__SenderChainNode *)malloc(sizeof(E2ees__SenderChainNode));
                    e2ees__sender_chain_node__init(new_sender_chain);

                    new_sender_chain->chain_key =
                        (E2ees__ChainKey *)malloc(sizeof(E2ees__ChainKey));
                    ...
                    if (cipher_suite->kem_suite->get_param().pqc_param == false) {
                        ...
                        cipher_suite->kem_suite->asym_key_gen(
                            &new_ratchet_key_pair->public_key, &new_ratchet_key_pair->private_key);

                        copy_protobuf_from_protobuf(
                            &(new_receiver_chain->our_ratchet_private_key),
                            &(new_ratchet_key_pair->private_key));

                        copy_protobuf_from_protobuf(
                            &(new_sender_chain->our_ratchet_public_key),
                            &(new_ratchet_key_pair->public_key));
                        copy_protobuf_from_protobuf(
                            &(new_sender_chain->their_ratchet_public_key),
                            &(new_receiver_chain->their_ratchet_public_key));
                        create_chain_key(
                            cipher_suite,
                            ratchet->root_key,
        ...
        if (ret == E2EES_RESULT_SUCC && corresponding_receiver_chain != NULL) {
            if (corresponding_receiver_chain->chain_key->index < payload->sequence) {
                 ...
                 * We will generate the corresponding message keys and store
                 * them together with their ratchet key in the skipped message
                 * key list. */
                size_t skipped_num =
                ...
                if (ratchet->n_skipped_msg_key_list + skipped_num > MAX_SKIPPED_MESSAGE_KEY_NODES) {
                    e2ees_notify_log(
                        NULL,
                        DEBUG_LOG,
                        "decrypt_ratchet(): Too many skipped keys in current "
                        "chain! Bomb defused.");
                    ret = E2EES_RESULT_FAIL;
                } else {
                    if (ratchet->skipped_msg_key_list == NULL) {
                        ...
                        if (ratchet->skipped_msg_key_list == NULL) {
                            e2ees_notify_log(
                                NULL,
                                DEBUG_LOG,
                                "decrypt_ratchet(): Memory allocation failed "
                                "for skipped_msg_key_list.");
                            ret = E2EES_RESULT_FAIL;
                        }
                    } else {
                        E2ees__SkippedMsgKeyNode **temp_skipped_message_keys =
                    ...
                    if (ret == E2EES_RESULT_SUCC) {
                        while (corresponding_receiver_chain->chain_key->index < payload->sequence) {
                            ...
                            copy_protobuf_from_protobuf(
                                &(key->ratchet_key_public),
                                &(corresponding_receiver_chain->their_ratchet_public_key));

                            ratchet->skipped_msg_key_list[ratchet->n_skipped_msg_key_list] = key;
                            (ratchet->n_skipped_msg_key_list)++;

                            if (advance_chain_key(
                                    cipher_suite, corresponding_receiver_chain->chain_key) !=
                                E2EES_RESULT_SUCC) {
                                ret = E2EES_RESULT_FAIL;
                                break;
                            }
            ...
            if (ret == E2EES_RESULT_SUCC) {
                if (corresponding_receiver_chain->chain_key->index == payload->sequence) {
                    /* If we decrypt the incoming message by a skipped message
