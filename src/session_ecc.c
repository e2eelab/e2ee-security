...
E2ees__InviteResponse *crypto_curve25519_new_outbound_session(
    ...
    if ((their_pre_key_bundle->identity_key_public->asym_public_key.len != key_len) ||
        ...
        (their_pre_key_bundle->signed_pre_key_public->signature.len !=
         cipher_suite->ds_suite->get_param().sig_len)) {
        e2ees_notify_log(
    ...
    cipher_suite->kem_suite->asym_key_gen(
        &my_ephemeral_key.public_key, &my_ephemeral_key.private_key);
    ...
    outbound_session->pre_shared_input_list =
        (ProtobufCBinaryData *)malloc(sizeof(ProtobufCBinaryData));
    copy_protobuf_from_protobuf(
        &(outbound_session->pre_shared_input_list[0]), &(my_ephemeral_key.public_key));
    ...
    if (their_pre_key_bundle->one_time_pre_key_public) {
        outbound_session->bob_one_time_pre_key_id =
            their_pre_key_bundle->one_time_pre_key_public->opk_id;
        x3dh_epoch = 4;
    }
    ...
    memcpy(
        (outbound_session->associated_data.data) + key_len,
        their_pre_key_bundle->identity_key_public->asym_public_key.data,
        key_len);
    ...
    cipher_suite->kem_suite->decaps(
        pos,
        &(my_identity_key_pair.private_key),
        &(their_pre_key_bundle->signed_pre_key_public->public_key));
    pos += shared_secret_len;
    cipher_suite->kem_suite->decaps(
        pos,
        &(my_ephemeral_key.private_key),
        &(their_pre_key_bundle->identity_key_public->asym_public_key));
    pos += shared_secret_len;
    cipher_suite->kem_suite->decaps(
        pos,
        &(my_ephemeral_key.private_key),
    ...
    if (x3dh_epoch == 4) {
        ...
        cipher_suite->kem_suite->decaps(
            pos,
            &(my_ephemeral_key.private_key),
            &(their_pre_key_bundle->one_time_pre_key_public->public_key));
    }
...
int crypto_curve25519_new_inbound_session(
    E2ees__Session *inbound_session, E2ees__Account *local_account, E2ees__InviteMsg *msg) {
    const cipher_suite_t *cipher_suite =
    ...
    if (local_account->signed_pre_key->spk_id != msg->bob_signed_pre_key_id) {
        get_e2ees_plugin()->db_handler.load_signed_pre_key(
            local_account->address, msg->bob_signed_pre_key_id, &old_spk_data);
    ...
    inbound_session->pre_shared_input_list =
        (ProtobufCBinaryData *)malloc(sizeof(ProtobufCBinaryData));
    copy_protobuf_from_protobuf(
        &(inbound_session->pre_shared_input_list[0]), &(msg->pre_shared_input_list[0]));
    ...
    memcpy(
        (inbound_session->associated_data.data) + key_len,
        local_account->identity_key->asym_key_pair->public_key.data,
        key_len);
    ...
    if (x3dh_epoch == 4) {
        ...
        if (!our_one_time_pre_key) {
            ...
            free_mem(
                (void **)&(inbound_session->pre_shared_input_list), sizeof(ProtobufCBinaryData));
            inbound_session->n_pre_shared_input_list = 0;
            free_mem(
                (void **)&(inbound_session->associated_data.data),
                inbound_session->associated_data.len);
            return E2EES_RESULT_FAIL;
        } else {
            mark_opk_as_used(local_account, our_one_time_pre_key->opk_id);
            get_e2ees_plugin()->db_handler.update_one_time_pre_key(
                local_account->address, our_one_time_pre_key->opk_id);
    ...
    uint8_t *pos = secret;
    cipher_suite->kem_suite->decaps(
        pos, &(bob_signed_pre_key->private_key), &msg->alice_identity_key);
    pos += shared_secret_len;
    cipher_suite->kem_suite->decaps(
        pos, &(bob_identity_key->private_key), &inbound_session->pre_shared_input_list[0]);
    pos += shared_secret_len;
    cipher_suite->kem_suite->decaps(
        pos, &(bob_signed_pre_key->private_key), &inbound_session->pre_shared_input_list[0]);
    if (x3dh_epoch == 4) {
        pos += shared_secret_len;
        cipher_suite->kem_suite->decaps(
            pos, &(bob_one_time_pre_key->private_key), &inbound_session->pre_shared_input_list[0]);
    }
    ...
    /** The one who sends the accept message will be the one who received the
     * invitation message. Thus, the "from" and "to" of acception message will
     * be different from those in the session.
     */
    int ret                         = E2EES_RESULT_SUCC;
