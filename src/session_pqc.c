...
int pqc_new_outbound_session_v2(
            ...
            if (is_valid_pre_key_bundle(their_pre_key_bundle)) {
                ...
                if (is_valid_cipher_suite(cipher_suite)) {
                    ...
                    e2ees_notify_log(
                        NULL,
                        BAD_CIPHER_SUITE,
                        "pqc_new_outbound_session_v2() invalid cipher suite "
                        "with e2ees_pack_id = %d",
                        e2ees_pack_id);
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        ret = cipher_suite->ds_suite->verify(
            ...
            their_spk->signature.len,
            their_spk->public_key.data,
            asym_pub_key_len,
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        outbound_session->ratchet->sender_chain =
        ...
        copy_protobuf_from_protobuf(
            &(outbound_session->ratchet->sender_chain->their_ratchet_public_key),
            &(their_spk->public_key));
        ...
        memcpy(
            outbound_session->associated_data.data,
            my_identity_key_pair->public_key.data,
            asym_pub_key_len);
        memcpy(
            (outbound_session->associated_data.data) + asym_pub_key_len,
            their_ik->asym_public_key.data,
            asym_pub_key_len);
        ...
        memcpy(
            hash_input + asym_pub_key_len + asym_pub_key_len,
            their_spk->public_key.data,
            asym_pub_key_len);
        if (x3dh_epoch == 3) {
            memcpy(
                hash_input + asym_pub_key_len + asym_pub_key_len + asym_pub_key_len,
                their_opk->public_key.data,
                asym_pub_key_len);
        }
        ...
        cipher_suite->hf_suite->hkdf(
            ...
            sizeof(FINGERPRINT_SEED) - 1,
            derived_secrets,
            sizeof(derived_secrets));

        copy_protobuf_from_array(
            &(outbound_session->fingerprint), derived_secrets, sizeof(derived_secrets));
        ...
        // the first part of the shared secret will be determined after
        // receiving the acception message
        ...
        outbound_session->temp_shared_secret.data =
            (uint8_t *)malloc(sizeof(uint8_t) * outbound_session->temp_shared_secret.len);
        memcpy(outbound_session->temp_shared_secret.data, zero_array, shared_secret_len);
        memcpy(
            outbound_session->temp_shared_secret.data + shared_secret_len,
            secret,
            x3dh_epoch * shared_secret_len);

        // prepare the encaps_ciphertext_list
        outbound_session->n_pre_shared_input_list = x3dh_epoch;
        outbound_session->pre_shared_input_list =
            (ProtobufCBinaryData *)malloc(sizeof(ProtobufCBinaryData) * x3dh_epoch);
        ...
        if (x3dh_epoch == 3) {
            ...
            copy_protobuf_from_protobuf(
                &(outbound_session->pre_shared_input_list[2]), &ciphertext_4);
        }
        ...
        cipher_suite->kem_suite->asym_key_gen(
            &outbound_session->alice_base_key->public_key,
            &outbound_session->alice_base_key->private_key);

        // store sesson state before send invite
        e2ees_notify_log(
            outbound_session->our_address,
            DEBUG_LOG,
            "pqc_new_outbound_session() store sesson state before send invite "
            "session_id=%s, from [%s:%s], to [%s:%s]",
...
int pqc_new_inbound_session(
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        if (our_spk->spk_id != bob_signed_pre_key_id) {
            get_e2ees_plugin()->db_handler.load_signed_pre_key(
                address, bob_signed_pre_key_id, &old_spk_data);
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        if (x3dh_epoch == 4) {
            ...
            get_e2ees_plugin()->db_handler.update_one_time_pre_key(
                address, our_one_time_pre_key->opk_id);
        ...
        memcpy(
            (session->associated_data.data) + asym_pub_key_len,
            our_ik->asym_key_pair->public_key.data,
            asym_pub_key_len);
        ...
        memcpy(
            hash_input + asym_pub_key_len,
            our_ik->asym_key_pair->public_key.data,
            asym_pub_key_len);
        memcpy(
            hash_input + asym_pub_key_len + asym_pub_key_len,
            our_spk->key_pair->public_key.data,
            asym_pub_key_len);
        if (x3dh_epoch == 4) {
            memcpy(
                hash_input + asym_pub_key_len + asym_pub_key_len + asym_pub_key_len,
                bob_one_time_pre_key->public_key.data,
                asym_pub_key_len);
        }
        uint8_t derived_secrets[2 * shared_key_len];
        uint8_t salt[hf_len];
        ...
        cipher_suite->kem_suite->decaps(
            pos, &(bob_identity_key->private_key), &(msg->pre_shared_input_list[0]));
        pos += shared_secret_len;
        cipher_suite->kem_suite->decaps(
            pos, &(bob_signed_pre_key->private_key), &(msg->pre_shared_input_list[1]));
        if (x3dh_epoch == 4) {
            pos += shared_secret_len;
            cipher_suite->kem_suite->decaps(
                pos, &(bob_one_time_pre_key->private_key), &(msg->pre_shared_input_list[2]));
        }

        ret = initialise_as_bob(
            &ratchet,
            cipher_suite,
            secret,
            sizeof(secret),
    ...
    if (ret == E2EES_RESULT_SUCC) {
        /** The one who sends the acception message will be the one who received
         * the invitation message. Thus, the "from" and "to" of acception
         * message will be different from those in the session. */
        ret = accept_internal(
            &accept_response,
...
int pqc_complete_outbound_session(E2ees__Session **outbound_session_out, E2ees__AcceptMsg *msg) {
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        copy_protobuf_from_protobuf(
            &their_ratchet_key, &(session->ratchet->sender_chain->their_ratchet_public_key));
        ...
        ret = initialise_as_alice(
            ...
            cipher_suite,
            session->temp_shared_secret.data,
            session->temp_shared_secret.len,
...
session_suite_t E2EES_SESSION_PQC = { pqc_new_outbound_session_v2,
