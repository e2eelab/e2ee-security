...
int publish_spk_internal(E2ees__PublishSpkResponse **response_out, uint32_t e2ees_pack_id, E2ees__E2eeAddress *user_address, E2ees__SignedPreKey *new_spk_pair) {
    int ret = E2EES_RESULT_SUCC;

    E2ees__PublishSpkRequest *publish_spk_request = NULL;
    E2ees__PublishSpkResponse *response = NULL;
    char *auth = NULL;
    publish_spk_params_t params = {.e2ees_pack_id = e2ees_pack_id, .user_address = user_address, .new_spk = new_spk_pair};
    ...
    if (ret == E2EES_RESULT_SUCC) {
        response = dispatch_proto_request(get_e2ees_plugin()->proto_handler.publish_spk, publish_spk_request, user_address, auth);
...
int supply_opks_internal(E2ees__SupplyOpksResponse **response_out, E2ees__Account *account, uint32_t opks_num) {
    int ret = E2EES_RESULT_SUCC;
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ret = generate_opks(&one_time_pre_key_list, (size_t)opks_num, account->e2ees_pack_id, account->next_one_time_pre_key_id);
    }
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        if (is_valid_supply_opks_response(response)) {
            ret = consume_supply_opks_response(account->address, one_time_pre_key_list, opks_num, response);
        }
...
int get_pre_key_bundle_internal(E2ees__InviteResponse ***invite_response_list_out, size_t *invite_response_num_out, E2ees__E2eeAddress *from, const char *auth, const char *to_user_id,
                                const char *to_domain, const char *to_device_id, bool active, uint8_t *group_pre_key_plaintext_data, size_t group_pre_key_plaintext_data_len) {
    int ret = E2EES_RESULT_SUCC;
    ...
    *invite_response_num_out = 0;
    ...
    get_pre_key_bundle_params_t params = {.from = from, .auth = auth, .to_user_id = to_user_id, .to_domain = to_domain, .to_device_id = to_device_id};
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ret = consume_get_pre_key_bundle_response(&invite_response_list, &invite_response_num, from, group_pre_key_plaintext_data, group_pre_key_plaintext_data_len, get_pre_key_bundle_response);
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        if (get_pre_key_bundle_response != NULL) {
            if (get_pre_key_bundle_response->code != E2EES__RESPONSE_CODE__RESPONSE_CODE_NOT_FOUND) {
                ...
                store_pending_request_internal(from, E2EES__PENDING_REQUEST_TYPE__PENDING_REQUEST_TYPE_GET_PRE_KEY_BUNDLE, get_pre_key_bundle_request_data, get_pre_key_bundle_request_data_len,
...
int invite_internal(E2ees__InviteResponse **response_out, E2ees__Session *outbound_session) {
    ...
    if (ret == E2EES_RESULT_SUCC) {
        response = dispatch_proto_request(get_e2ees_plugin()->proto_handler.invite, invite_request, outbound_session->our_address, auth);
        ...
        if (ret == E2EES_RESULT_FAIL) {
            ...
            store_pending_request_internal(outbound_session->our_address, E2EES__PENDING_REQUEST_TYPE__PENDING_REQUEST_TYPE_INVITE, request_data, request_data_len, NULL, 0);
            // release
...
int accept_internal(E2ees__AcceptResponse **response_out, uint32_t e2ees_pack_id, E2ees__E2eeAddress *from, E2ees__E2eeAddress *to, char *session_id, ProtobufCBinaryData *ciphertext_1,
    ...
    char *auth = NULL;
    accept_params_t params = {.e2ees_pack_id = e2ees_pack_id, .from = from, .to = to, .session_id = session_id, .ciphertext_1 = ciphertext_1, .our_ratchet_key = our_ratchet_key};
    ...
    if (ret == E2EES_RESULT_SUCC) {
        response = dispatch_proto_request(get_e2ees_plugin()->proto_handler.accept, accept_request, from, auth);
...
E2ees__SendOne2oneMsgResponse *send_one2one_msg_internal(E2ees__Session *outbound_session, uint32_t notif_level, const uint8_t *plaintext_data, size_t plaintext_data_len) {
    int ret = E2EES_RESULT_SUCC;
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        ret = encrypt_ratchet(&payload_out, cipher_suite, outbound_session->ratchet, outbound_session->associated_data, plaintext_data, plaintext_data_len);
    }
    ...
    if (ret == E2EES_RESULT_SUCC) {
        response = dispatch_proto_request(get_e2ees_plugin()->proto_handler.send_one2one_msg, send_one2one_msg_request, outbound_session->our_address, auth);
        ...
        if (!succ) {
            ...
            store_pending_request_internal(outbound_session->our_address, E2EES__PENDING_REQUEST_TYPE__PENDING_REQUEST_TYPE_SEND_ONE2ONE_MSG, request_data, request_data_len, NULL, 0);
...
int add_group_member_device_internal(E2ees__AddGroupMemberDeviceResponse **response_out, E2ees__E2eeAddress *sender_address, E2ees__E2eeAddress *group_address,
    ...
    add_group_member_device_params_t params = {.sender_address = sender_address, .group_address = group_address, .new_device_address = new_device_address};
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        if (!is_valid_add_group_member_device_response(response)) {
            ...
            store_pending_request_internal(sender_address, E2EES__PENDING_REQUEST_TYPE__PENDING_REQUEST_TYPE_ADD_GROUP_MEMBER_DEVICE, request_data, request_data_len, NULL, 0);
            // release
            free_mem((void **)&request_data, request_data_len);
        }
...
void store_pending_common_plaintext_data_internal(E2ees__E2eeAddress *from, E2ees__E2eeAddress *to, uint8_t *common_plaintext_data, size_t common_plaintext_data_len,
                                                  E2ees__NotifLevel notif_level) {
    char *pending_plaintext_id = generate_uuid_str();
    get_e2ees_plugin()->db_handler.store_pending_plaintext_data(from, to, pending_plaintext_id, common_plaintext_data, common_plaintext_data_len, notif_level);

    // release
    free(pending_plaintext_id);
}

void store_pending_request_internal(E2ees__E2eeAddress *user_address, E2ees__PendingRequestType type, uint8_t *request_data, size_t request_data_len, ProtobufCBinaryData *request_arg_list,
...
static void resend_pending_request(const uint32_t e2ees_pack_id, E2ees__E2eeAddress *user_address, const char *auth) {
    ...
    size_t pending_request_data_num =
        get_e2ees_plugin()->db_handler.load_pending_request_data(user_address, &pending_request_id_list, &request_type_list, &request_data_list, &request_data_len_list);
    ...
    for (i = 0; i < pending_request_data_num; i++) {
        ...
        if (pending_request == NULL) {
            e2ees_notify_log(user_address, DEBUG_LOG, "resend_pending_request() unpack failed for index %zu", i);
        ...
        e2ees_notify_log(user_address, DEBUG_LOG, "resend_pending_request() request_type: %d", request_type_list[i]);
        ...
        case E2EES__PENDING_REQUEST_TYPE__PENDING_REQUEST_TYPE_GET_PRE_KEY_BUNDLE: {
            ...
            if (ret == E2EES_RESULT_SUCC) {
                ...
                ret = consume_get_pre_key_bundle_response(&invite_response_list, &invite_response_num, user_address, group_pre_key_plaintext_data, group_pre_key_plaintext_data_len,
                ...
                if (ret == E2EES_RESULT_SUCC) {
                    if (pending_request->n_request_arg_list > 0 && pending_request->request_arg_list[0].data != NULL && pending_request->request_arg_list[0].data[0] == 'T') {
                        their_device_num = get_pre_key_bundle_response->n_pre_key_bundles;
                        char **their_device_id = (char **)malloc(sizeof(char *) * their_device_num);
                        E2ees__PreKeyBundle *cur_pre_key_bundle = NULL;
                        ...
                        // send to other devices in order to create sessions
                        send_sync_invite_msg(user_address, get_pre_key_bundle_request->user_id, get_pre_key_bundle_request->domain, their_device_id, their_device_num);
                ...
                if (get_pre_key_bundle_response != NULL) {
                    if (get_pre_key_bundle_response->code == E2EES__RESPONSE_CODE__RESPONSE_CODE_NOT_FOUND) {
                        e2ees_notify_log(user_address, DEBUG_LOG, "consume_get_pre_key_bundle_response() got empty pre_key_bundles, remove pending request");
                        succ = true;
                    }
                }
            }
        ...
        case E2EES__PENDING_REQUEST_TYPE__PENDING_REQUEST_TYPE_SUPPLY_OPKS: {
            ...
            if (succ) {
                ...
                ret = consume_supply_opks_response(user_address, one_time_pre_key_list, opks_num, supply_opks_response);
                if (ret == E2EES_RESULT_SUCC) {
                    get_e2ees_plugin()->db_handler.unload_pending_request_data(user_address, pending_request_id_list[i]);
                }
                // release
        ...
        case E2EES__PENDING_REQUEST_TYPE__PENDING_REQUEST_TYPE_CREATE_GROUP: {
            ...
            if (succ) {
                ret = consume_create_group_response(e2ees_pack_id, user_address, create_group_request->msg->group_info->group_name,
                                                    create_group_request->msg->group_info->group_member_list, create_group_request->msg->group_info->n_group_member_list, create_group_response);
                get_e2ees_plugin()->db_handler.unload_pending_request_data(user_address, pending_request_id_list[i]);
            } else {
        ...
        case E2EES__PENDING_REQUEST_TYPE__PENDING_REQUEST_TYPE_ADD_GROUP_MEMBERS: {
            ...
            if (succ) {
                ...
                get_e2ees_plugin()->db_handler.load_group_session_by_address(user_address, user_address, add_group_members_msg->group_info->group_address, &group_session);
                ret = consume_add_group_members_response(group_session, add_group_members_response, add_group_members_msg->adding_member_list, add_group_members_msg->n_adding_member_list);
                get_e2ees_plugin()->db_handler.unload_pending_request_data(user_address, pending_request_id_list[i]);
            } else {
        ...
        case E2EES__PENDING_REQUEST_TYPE__PENDING_REQUEST_TYPE_ADD_GROUP_MEMBER_DEVICE: {
            E2ees__AddGroupMemberDeviceRequest *add_group_member_device_request =
                e2ees__add_group_member_device_request__unpack(NULL, pending_request->request_data.len, pending_request->request_data.data);
            ...
            if (succ) {
                get_e2ees_plugin()->db_handler.load_group_session_by_address(user_address, user_address, add_group_member_device_request->msg->group_info->group_address, &group_session);
                ret = consume_add_group_member_device_response(group_session, add_group_member_device_response);
                get_e2ees_plugin()->db_handler.unload_pending_request_data(user_address, pending_request_id_list[i]);
            } else {
        ...
        case E2EES__PENDING_REQUEST_TYPE__PENDING_REQUEST_TYPE_REMOVE_GROUP_MEMBERS: {
            E2ees__RemoveGroupMembersRequest *remove_group_members_request =
                e2ees__remove_group_members_request__unpack(NULL, pending_request->request_data.len, pending_request->request_data.data);
            ...
            if (succ) {
                ...
                get_e2ees_plugin()->db_handler.load_group_session_by_address(user_address, user_address, remove_group_members_msg->group_info->group_address, &group_session);
                ret = consume_remove_group_members_response(group_session, remove_group_members_response, remove_group_members_msg->removing_member_list,
        ...
        default:
            e2ees_notify_log(user_address, DEBUG_LOG, "resend_pending_request() unknown pending request type: %d, %s", request_type_list[i]);
            break;
        };
        // release
        e2ees__pending_request__free_unpacked(pending_request, NULL);
