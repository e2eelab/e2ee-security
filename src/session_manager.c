...
static void send_pending_plaintext_data(E2ees__Session *outbound_session) {
    // load pending plaintext data(may be the group pre-key or the common
    // plaintext)
    ...
    pending_plaintext_data_list_num = get_e2ees_plugin()->db_handler.load_pending_plaintext_data(
    ...
    if (pending_plaintext_data_list_num > 0) {
        ...
        for (i = 0; i < pending_plaintext_data_list_num; i++) {
            free_mem(
                (void **)&(pending_plaintext_data_list[i]), pending_plaintext_data_len_list[i]);
            free(pending_plaintext_id_list[i]);
        }
        free_mem(
            (void **)&pending_plaintext_id_list, sizeof(char *) * pending_plaintext_data_list_num);
        free_mem(
            (void **)&pending_plaintext_data_list,
            sizeof(uint8_t *) * pending_plaintext_data_list_num);
        free_mem(
            (void **)&pending_plaintext_data_len_list,
            sizeof(size_t) * pending_plaintext_data_list_num);
        free_mem(
            (void **)&notif_level_list,
            sizeof(E2ees__NotifLevel) * pending_plaintext_data_list_num);
    }
}
...
int produce_get_pre_key_bundle_request(
    ...
    request->device_id =
        params->to_device_id ? strdup(params->to_device_id) : strdup(""); // device_id may be NULL
...
int consume_get_pre_key_bundle_response(
    ...
    if (is_valid_address(from)) {
        ...
        if (is_valid_get_pre_key_bundle_response(get_pre_key_bundle_response)) {
            ...
            e2ees_notify_log(
                ...
                "consume_get_pre_key_bundle_response(): invalid "
        ...
        for (i = 0; i < n_pre_key_bundles; i++) {
            ...
            if (is_valid_server_signed_signature(cur_pre_key_bundle->signature)) {
                ...
                if (server_public_key.data != NULL) {
                    ds_suite_t *digital_signature_suite =
                        get_ds_suite(cur_pre_key_bundle->signature->signing_alg);
                    int server_check = digital_signature_suite->verify(
                    ...
                    if (server_check == E2EES_RESULT_FAIL) {
                        e2ees_notify_log(
                            NULL,
                            BAD_SERVER_SIGNATURE,
                            "consume_get_pre_key_bundle_response() verify "
                            "failed");
                        ret = E2EES_RESULT_FAIL;
                    }
                } else {
                    e2ees_notify_log(
                        NULL,
                        BAD_SERVER_SIGNATURE,
                        "consume_get_pre_key_bundle_response() "
                        "server_public_key is null");
                    ret = E2EES_RESULT_FAIL;
                }
            } else {
                e2ees_notify_log(
                    NULL,
                    BAD_SERVER_SIGNATURE,
                    "consume_get_pre_key_bundle_response() invalid signature");
                ret = E2EES_RESULT_FAIL;
            }
        }
    }
    ...
    if (ret == E2EES_RESULT_SUCC) {
        for (i = 0; i < n_pre_key_bundles; i++) {
            ...
            if (is_self) {
                // the same device_id
                if (compare_address(from, their_pre_key_bundles[i]->user_address)) {
                    continue;
                }
            }
            invite_response_list_len = invite_response_list_len + 1;
        }
        invite_response_list = (E2ees__InviteResponse **)malloc(
            sizeof(E2ees__InviteResponse *) * invite_response_list_len);
        ...
        for (i = 0; i < n_pre_key_bundles; i++) {
            ...
            if (group_pre_key_plaintext_data != NULL) {
                e2ees_notify_log(
                    from,
                    DEBUG_LOG,
                    "consume_get_pre_key_bundle_response() store the group "
                    "pre-keys");
            ...
            invite_response_ret                  = session_suite->new_outbound_session(
                &(invite_response_list[insert_pos]), from, cur_pre_key_bundle);
            insert_pos++;
        }
...
int produce_send_one2one_msg_request(
    ...
    E2ees__SendOne2oneMsgRequest *request =
        (E2ees__SendOne2oneMsgRequest *)malloc(sizeof(E2ees__SendOne2oneMsgRequest));
...
bool consume_send_one2one_msg_response(
    ...
    // store sesson state
    if (remove_session) {
        get_e2ees_plugin()->db_handler.unload_session(
...
bool consume_one2one_msg(E2ees__E2eeAddress *receiver_address, E2ees__E2eeMsg *e2ee_msg) {
    ...
    if (is_valid_e2ee_msg(e2ee_msg)) {
        ...
        get_e2ees_plugin()->db_handler.load_session(
            e2ee_msg->session_id, receiver_address, &inbound_session);
        if (!is_valid_inbound_session(inbound_session)) {
            e2ees_notify_log(
                receiver_address, BAD_SESSION, "consume_one2one_msg(), invalid inbound session");
            ret = E2EES_RESULT_FAIL;
        }
    } else {
        e2ees_notify_log(receiver_address, BAD_E2EE_MSG, "consume_one2one_msg(), invalid e2ee_msg");
        ret = E2EES_RESULT_FAIL;
    }
    ...
    if (e2ee_msg->payload_case == E2EES__E2EE_MSG__PAYLOAD_ONE2ONE_MSG) {
        ...
        e2ees_notify_log(
            receiver_address, BAD_E2EE_MSG, "consume_one2one_msg(): invalid payload_case");
        ret = E2EES_RESULT_FAIL;
    }

    if (ret == E2EES_RESULT_SUCC) {
        const cipher_suite_t *cipher_suite =
            get_e2ees_pack(inbound_session->e2ees_pack_id)->cipher_suite;
        ret = decrypt_ratchet(
            &decrypted_data_out,
            &decrypted_data_len_out,
            cipher_suite,
            inbound_session->ratchet,
            inbound_session->associated_data,
            payload);
    }
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        get_e2ees_plugin()->db_handler.unload_old_session(
            receiver_address, e2ee_msg->from, inbound_session->invite_t);

        E2ees__Plaintext *plaintext =
            e2ees__plaintext__unpack(NULL, decrypted_data_len_out, decrypted_data_out);
        if (plaintext != NULL) {
            if (plaintext->payload_case == E2EES__PLAINTEXT__PAYLOAD_COMMON_MSG) {
                e2ees_notify_one2one_msg(
                    receiver_address,
                    e2ee_msg->from,
                    e2ee_msg->to,
                    plaintext->common_msg.data,
                    plaintext->common_msg.len);
                ...
                e2ees_notify_other_device_msg(
                    receiver_address,
                    e2ee_msg->from,
                    e2ee_msg->to,
                    plaintext->common_sync_msg.data,
                    plaintext->common_sync_msg.len);
                ...
                if (group_pre_key_bundle->old_session_id != NULL &&
                    group_pre_key_bundle->old_session_id[0] != '\0') {
                    get_e2ees_plugin()->db_handler.unload_group_session_by_id(
                        receiver_address, group_pre_key_bundle->old_session_id);
                    e2ees_notify_log(
                        receiver_address,
                        DEBUG_LOG,
                        "unload the old outbound and inbound group sessions "
                        "session_id: %s, session_owner: [%s:%s]",
                ...
                size_t inbound_group_sessions_num =
                    get_e2ees_plugin()->db_handler.load_group_sessions(
                        receiver_address,
                        group_pre_key_bundle->group_info->group_address,
                        &inbound_group_sessions);
                e2ees_notify_log(
                    receiver_address,
                    DEBUG_LOG,
                    "consume_one2one_msg() "
                    "E2EES__PLAINTEXT__PAYLOAD_GROUP_PRE_KEY_BUNDLE : "
                    "inbound_group_sessions_num: %d, inbound_group_sessions: "
                    "%p",
                ...
                if (inbound_group_sessions_num > 0 && inbound_group_sessions != NULL) {
                    ...
                    for (i = 0; i < inbound_group_sessions_num; i++) {
                        ...
                        complete_inbound_group_session_by_pre_key_bundle(
                            inbound_group_sessions[i], group_pre_key_bundle);
                        e2ees_notify_log(
                            receiver_address,
                            DEBUG_LOG,
                            "complete_inbound_group_session_by_pre_key_bundle: "
                    ...
                    e2ees_notify_log(
                        receiver_address,
                        DEBUG_LOG,
                        "new_outbound_group_session_by_receiver: %s, "
                        "session_owner: [%s:%s]",
                        group_pre_key_bundle->session_id,
                        receiver_address->user->user_id,
                        receiver_address->user->device_id);
                } else {
                    // the sender in the inbound group session is the one who
                    // creates the group
                    new_inbound_group_session_by_pre_key_bundle(
                        group_pre_key_bundle->e2ees_pack_id,
                        receiver_address,
                    ...
                    e2ees_notify_log(
                        receiver_address,
                        DEBUG_LOG,
                        "new_inbound_group_session_by_pre_key_bundle: %s, "
                        "session_owner: [%s:%s]",
                        group_pre_key_bundle->session_id,
                        receiver_address->user->user_id,
                        receiver_address->user->device_id);
                ...
                E2ees__GroupUpdateKeyBundle *group_update_key_bundle =
                    plaintext->group_update_key_bundle;
                ...
                if (group_update_key_bundle->adding == true) {
                    ...
                    e2ees_notify_log(
                        receiver_address,
                        DEBUG_LOG,
                        "new_outbound_group_session_invited: %s, "
                        "session_owner: [%s:%s]",
                        group_update_key_bundle->session_id,
                        receiver_address->user->user_id,
                        receiver_address->user->device_id);
                }
                new_and_complete_inbound_group_session_with_ratchet_state(
                    group_update_key_bundle, receiver_address);
                e2ees_notify_log(
                    receiver_address,
                    DEBUG_LOG,
                    "new_and_complete_inbound_group_session_with_ratchet_state:"
            ...
            e2ees_notify_log(
                receiver_address,
                BAD_PLAINTEXT,
                "consume_one2one_msg(), plaintext data unpack error");
            ret = E2EES_RESULT_FAIL;
        }
        // release
        ...
        e2ees_notify_log(
            receiver_address, BAD_MESSAGE_DECRYPTION, "consume_one2one_msg() decryption failed");
        ret = E2EES_RESULT_FAIL;
    }
...
bool consume_add_user_device_msg(
    ...
    if (is_valid_add_user_device_msg(msg)) {
        ...
        e2ees_notify_log(
            NULL, BAD_ADD_USER_DEVICE_MSG, "consume_add_user_device_msg(): no add_user_device_msg");
        ret = E2EES_RESULT_FAIL;
    }
    ...
    if (compare_address(receiver_address, new_user_address)) {
        e2ees_notify_log(
            NULL,
            BAD_ADDRESS,
            "consume_add_user_device_msg(): receiver_address and "
            "new_user_address are the same");
        ret = E2EES_RESULT_FAIL;
    }
    ...
    for (i = 0; i < old_address_list_number; i++) {
        ...
        group_address_num = get_e2ees_plugin()->db_handler.load_group_addresses(
            old_address_list[i], receiver_address, &group_addresses);
        ...
        for (j = 0; j < group_address_num; j++) {
            if (group_address_list != NULL) {
                ...
                while (cur_group_address_node != NULL) {
                    if (compare_address(
                            cur_group_address_node->group_address, group_addresses[j])) {
                        break;
                    }
                ...
                if (tail_group_address_node != NULL) {
                    tail_group_address_node->next =
                        (group_address_node *)malloc(sizeof(group_address_node));
                    copy_address_from_address(
                        &(tail_group_address_node->next->group_address), group_addresses[j]);
    ...
    if (group_address_list != NULL) {
        ...
        while (cur_group_address_node != NULL) {
            ...
            ret = add_group_member_device_internal(
                ...
                receiver_address,
                cur_group_address_node->group_address,
                new_user_address);
...
int produce_invite_request(E2ees__InviteRequest **request_out, E2ees__Session *outbound_session) {
    ...
    msg->pre_shared_input_list =
        (ProtobufCBinaryData *)malloc(sizeof(ProtobufCBinaryData) * msg->n_pre_shared_input_list);
    size_t i;
    for (i = 0; i < msg->n_pre_shared_input_list; i++) {
        init_protobuf(&(msg->pre_shared_input_list[i]));
        copy_protobuf_from_protobuf(
            &(msg->pre_shared_input_list[i]), &(outbound_session->pre_shared_input_list[i]));
    }

    copy_protobuf_from_protobuf(
        &(msg->alice_base_key), &(outbound_session->alice_base_key->public_key));
...
int consume_invite_response(E2ees__E2eeAddress *user_address, E2ees__InviteResponse *response) {
    ...
    if (is_valid_address(user_address)) {
        if (is_valid_invite_response(response)) {
            ...
            e2ees_notify_log(
                NULL, BAD_INVITE_RESPONSE, "consume_invite_response(): invalid invite_response");
            ret = E2EES_RESULT_FAIL;
        }
    } else {
    ...
    if (succ) {
        ...
        E2ees__Session *inbound_session = NULL;
        get_e2ees_plugin()->db_handler.load_session(
            response->session_id, user_address, &inbound_session);
        if (inbound_session != NULL) {
            // update invite_t
            ...
            e2ees_notify_log(
                NULL, BAD_SESSION, "consume_invite_response(): invalid inbound_session");
            ret = E2EES_RESULT_FAIL;
        }
...
bool consume_invite_msg(E2ees__E2eeAddress *receiver_address, E2ees__InviteMsg *invite_msg) {
    ...
    if (is_valid_address(receiver_address)) {
        if (is_valid_invite_msg(invite_msg)) {
            e2ees_pack_id = invite_msg->e2ees_pack_id;
            from          = invite_msg->from;
            to            = invite_msg->to;
            version       = invite_msg->version;
            session_id    = invite_msg->session_id;
            if (!compare_address(receiver_address, to)) {
                e2ees_notify_log(
                    receiver_address, BAD_ADDRESS, "consume_invite_msg() wrong receiver_address");
                ret = E2EES_RESULT_FAIL;
            }
        } else {
            e2ees_notify_log(NULL, BAD_INVITE_MSG, "consume_invite_msg() invalid invite_msg");
            ret = E2EES_RESULT_FAIL;
        }
    } else {
        e2ees_notify_log(NULL, BAD_ADDRESS, "consume_invite_msg() invalid receiver_address");
        ret = E2EES_RESULT_FAIL;
    }
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        if (inbound_session != NULL) {
            e2ees_notify_log(
                receiver_address,
                BAD_SESSION,
                "consume_invite_msg() session ID has been used, no need to "
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        if (!is_valid_registered_account(account)) {
            e2ees_notify_log(receiver_address, BAD_ACCOUNT, "consume_invite_msg() invalid account");
            ret = E2EES_RESULT_FAIL;
        }
    }
    ...
    if (ret == E2EES_RESULT_SUCC) {
        const session_suite_t *session_suite = get_e2ees_pack(e2ees_pack_id)->session_suite;
        // create a new inbound session
        ret = session_suite->new_inbound_session(&inbound_session, account, invite_msg);
    }
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        e2ees_notify_log(
            receiver_address,
            BAD_SESSION,
            "consume_invite_msg() failed to create a new inbound session");
    }
...
int consume_accept_response(E2ees__E2eeAddress *user_address, E2ees__AcceptResponse *response) {
    ...
    if (is_valid_address(user_address)) {
        if (is_valid_accept_response(response)) {
            ...
            e2ees_notify_log(
                NULL, BAD_ACCEPT_RESPONSE, "consume_accept_response(): invalid accept_response");
            ret = E2EES_RESULT_FAIL;
        }
    } else {
...
bool consume_accept_msg(E2ees__E2eeAddress *receiver_address, E2ees__AcceptMsg *accept_msg) {
    ...
    if (is_valid_address(receiver_address)) {
        if (is_valid_accept_msg(accept_msg)) {
            if (!compare_address(receiver_address, accept_msg->to)) {
                e2ees_notify_log(
                    receiver_address, BAD_ADDRESS, "consume_accept_msg() wrong receiver_address");
                ret = E2EES_RESULT_FAIL;
            }
        } else {
            e2ees_notify_log(NULL, BAD_ACCEPT_MSG, "consume_accept_msg() invalid accept_msg");
            ret = E2EES_RESULT_FAIL;
        }
    } else {
        e2ees_notify_log(NULL, BAD_ADDRESS, "consume_accept_msg() invalid receiver_address");
        ret = E2EES_RESULT_FAIL;
    }

    if (ret == E2EES_RESULT_SUCC) {
        const session_suite_t *session_suite =
            get_e2ees_pack(accept_msg->e2ees_pack_id)->session_suite;
        ret = session_suite->complete_outbound_session(&outbound_session, accept_msg);
    }
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        e2ees_notify_log(
            receiver_address, DEBUG_LOG, "consume_accept_msg(): complete_outbound_session error");
    }
