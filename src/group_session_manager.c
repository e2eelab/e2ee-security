...
int produce_create_group_request(
    ...
    copy_group_members(
        &(group_info->group_member_list), params->group_members, params->group_members_num);
...
int consume_create_group_response(
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ret = new_outbound_group_session_by_sender(
            response->n_member_info_list,
            response->member_info_list,
            e2ees_pack_id,
            sender_address,
            group_name,
            group_address,
    ...
    if (ret == E2EES_RESULT_SUCC) {
        // notify
        e2ees_notify_group_created(
            sender_address, group_address, group_name, group_member_list, group_members_num);
    } else {
...
bool consume_create_group_msg(E2ees__E2eeAddress *receiver_address, E2ees__CreateGroupMsg *msg) {
    ...
    if (is_valid_address(receiver_address)) {
        if (is_valid_create_group_msg(msg)) {
            ...
            e2ees_notify_log(
                NULL, BAD_CREATE_GROUP_MSG, "consume_create_group_msg() invalid create_group_msg");
            ret = E2EES_RESULT_FAIL;
        }
    } else {
    ...
    if (ret == E2EES_RESULT_SUCC) {
        get_e2ees_plugin()->db_handler.load_group_session_by_address(
            sender_address, receiver_address, group_address, &inbound_group_session);
        if (inbound_group_session != NULL) {
            ...
            if (!is_valid_group_session_by_pre_key_bundle(inbound_group_session)) {
                e2ees_notify_log(
                    NULL,
                    BAD_GROUP_SESSION,
                    "consume_create_group_msg() invalid inbound_group_session");
                ret = E2EES_RESULT_FAIL;
            }
        } else {
        ...
        if (inbound_group_session_exist == false) {
            ...
            for (i = 0; i < msg->n_member_info_list; i++) {
                ...
                if (!compare_address(cur_group_member_info->member_address, receiver_address)) {
                    ret = new_inbound_group_session_by_member_id(
                        e2ees_pack_id, receiver_address, cur_group_member_info, group_info);
                    if (ret == E2EES_RESULT_FAIL) {
                        e2ees_notify_log(
                            receiver_address,
                            BAD_GROUP_SESSION,
                            "consume_create_group_msg() "
                            "new_inbound_group_session_by_member_id failed "
                            "with member_address: [%s:%s]",
            ...
            for (i = 0; i < msg->n_member_info_list; i++) {
                ...
                if (!compare_address(cur_group_member_info->member_address, sender_address)) {
                    if (!compare_address(cur_group_member_info->member_address, receiver_address)) {
                        // the senders in the inbound group sessions are the
                        // members in the group, except for the one who creates
                        // the group
                        ret = new_and_complete_inbound_group_session(
                            cur_group_member_info, inbound_group_session);
                        if (ret == E2EES_RESULT_FAIL) {
                            e2ees_notify_log(
                                receiver_address,
                                DEBUG_LOG,
                                "consume_create_group_msg() "
                                "new_and_complete_inbound_group_session failed "
                                "with member_address: [%s:%s]",
                                cur_group_member_info->member_address->user->user_id,
                                cur_group_member_info->member_address->user->device_id);
                        }
                    ...
                    // the sender in this inbound group session is the one who
                    // creates the group
                    ret = complete_inbound_group_session_by_member_id(
                        inbound_group_session, cur_group_member_info);
                    if (ret == E2EES_RESULT_FAIL) {
                        e2ees_notify_log(
                            receiver_address,
                            DEBUG_LOG,
                            "consume_create_group_msg() "
                            "complete_inbound_group_session_by_member_id "
                            "failed with member_address: [%s:%s]",
                            cur_group_member_info->member_address->user->user_id,
                            cur_group_member_info->member_address->user->device_id);
                    }
                }
            ...
            if (ret == E2EES_RESULT_FAIL) {
                e2ees_notify_log(
                    ...
                    DEBUG_LOG,
                    "consume_create_group_msg() "
                    "new_outbound_group_session_by_receiver failed with "
                    "session_owner and sender_address: [%s:%s]",
    ...
    if (ret == E2EES_RESULT_SUCC) {
        // notify
        e2ees_notify_group_created(
            receiver_address, group_address, group_name, group_member_list, group_members_num);
    } else {
...
int produce_add_group_members_request(
    ...
    copy_group_members(
        &(msg->adding_member_list), params->adding_members, params->adding_members_num);
    add_group_members_to_group_info(
        &(msg->group_info),
        outbound_group_session->group_info,
        params->adding_members,
        params->adding_members_num);
...
int consume_add_group_members_response(
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        ret = renew_outbound_group_session_by_welcome_and_add(
            ...
            NULL,
            session_owner,
            response->n_adding_member_info_list,
            response->adding_member_info_list,
            added_members_num,
            added_members);
    }
...
bool consume_add_group_members_msg(
    E2ees__E2eeAddress *receiver_address, E2ees__AddGroupMembersMsg *msg) {
    int ret                                = E2EES_RESULT_SUCC;
    ...
    if (is_valid_address(receiver_address)) {
        if (is_valid_add_group_members_msg(msg)) {
            ...
            e2ees_notify_log(
                ...
                "consume_add_group_members_msg() invalid "
                "add_group_members_msg");
            ret = E2EES_RESULT_FAIL;
        }
    } else {
        e2ees_notify_log(
            NULL, BAD_ADDRESS, "consume_add_group_members_msg() invalid receiver_address");
        ret = E2EES_RESULT_FAIL;
    }

    /** The old group members have their own outbound group sessions, so they
     * need to renew them. On the other hand, the new group members need to
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        ret = renew_outbound_group_session_by_welcome_and_add(
            ...
            msg->sender_address,
            msg->n_adding_member_info_list,
            msg->adding_member_info_list,
...
int produce_add_group_member_device_request(
    ...
    request =
        (E2ees__AddGroupMemberDeviceRequest *)malloc(sizeof(E2ees__AddGroupMemberDeviceRequest));
    ...
    copy_address_from_address(
        &(msg->adding_member_device->member_address), params->new_device_address);
...
int consume_add_group_member_device_response(
    E2ees__GroupSession *outbound_group_session, E2ees__AddGroupMemberDeviceResponse *response) {
    int ret                                           = E2EES_RESULT_SUCC;
    ...
    if (ret == E2EES_RESULT_SUCC) {
        e2ees_notify_log(
            ...
            DEBUG_LOG,
            "consume_add_group_member_device_response() success, "
            "new_device_address: [%s:%s], group_address:[%s@%s]",
        ...
        e2ees_notify_log(
            session_owner, DEBUG_LOG, "consume_add_group_member_device_response() failed");
    }
...
bool consume_add_group_member_device_msg(
    E2ees__E2eeAddress *receiver_address, E2ees__AddGroupMemberDeviceMsg *msg) {
    int ret                                = E2EES_RESULT_SUCC;
    ...
    if (is_valid_address(receiver_address)) {
        if (is_valid_add_group_member_device_msg(msg)) {
            ...
            e2ees_notify_log(
                ...
                "consume_add_group_member_device_msg() invalid "
                "add_group_member_device_msg");
            ret = E2EES_RESULT_FAIL;
        }
    } else {
        e2ees_notify_log(
            NULL, BAD_ADDRESS, "consume_add_group_member_device_msg() invalid receiver_address");
        ret = E2EES_RESULT_FAIL;
    }
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        if (is_valid_group_session(outbound_group_session)) {
            ...
            if (!is_valid_group_session(inbound_group_session)) {
                e2ees_notify_log(
                    receiver_address, BAD_GROUP_SESSION, "consume_add_group_member_device_msg()");
                ret = E2EES_RESULT_FAIL;
            }
        } else {
            e2ees_notify_log(
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        ret = renew_group_sessions_with_new_device(
            ...
            &(inbound_group_session->chain_key),
            msg->sender_address,
            msg->adding_member_device->member_address,
...
int produce_remove_group_members_request(
    ...
    remove_group_members_from_group_info(
        &(msg->group_info),
        outbound_group_session->group_info,
        params->removing_members,
        params->removing_members_num);

    msg->n_removing_member_list = params->removing_members_num;
    copy_group_members(
        &(msg->removing_member_list), params->removing_members, params->removing_members_num);
...
int consume_remove_group_members_response(
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        if (group_members_num > 0 &&
            user_in_group(sender_address, group_member_list, group_members_num)) {
            ...
            e2ees_notify_log(
                sender_address,
                DEBUG_LOG,
                "consume_remove_group_members_response() skip renew outbound "
                "group session since user is not in group");
        }
    }
...
bool consume_remove_group_members_msg(
    E2ees__E2eeAddress *receiver_address, E2ees__RemoveGroupMembersMsg *msg) {
    int ret = E2EES_RESULT_SUCC;
    ...
    if (is_valid_address(receiver_address)) {
        if (is_valid_remove_group_members_msg(msg)) {
            ...
            e2ees_notify_log(
                ...
                "consume_remove_group_members_msg() invalid "
                "remove_group_members_msg");
            ret = E2EES_RESULT_FAIL;
        }
    } else {
        e2ees_notify_log(
            NULL, BAD_ADDRESS, "consume_remove_group_members_msg() invalid receiver_address");
        ret = E2EES_RESULT_FAIL;
    }

    if (ret == E2EES_RESULT_SUCC) {
        // if the receiver is the one who is going to be removed, the receiver
        // should unload his or her own group session
        for (i = 0; i < msg->n_removing_member_list; i++) {
            if (safe_strcmp(
                    receiver_address->user->user_id, msg->removing_member_list[i]->user_id) &&
                safe_strcmp(receiver_address->domain, msg->removing_member_list[i]->domain)) {
                // unload all outbound and inbound group sessions
                get_e2ees_plugin()->db_handler.unload_group_session_by_address(
                    receiver_address, group_address);
                ...
                e2ees_notify_log(
                    receiver_address,
                    DEBUG_LOG,
                    "consume_remove_group_members_msg() skip renew outbound "
                    "group session because local user is removed");
                removed_member = true;
            }
        }
    }
    ...
    if ((ret == E2EES_RESULT_SUCC) && (removed_member == false)) {
        ...
        if (is_valid_group_session(inbound_group_session)) {
            if (!compare_group_member(
                    inbound_group_session->group_info->group_member_list,
                    inbound_group_session->group_info->n_group_member_list,
                    new_group_members,
                    new_group_members_num)) {
                new_group_session = false;
                // unload the old group sessions
                get_e2ees_plugin()->db_handler.unload_group_session_by_id(
                    receiver_address, inbound_group_session->session_id);
            }
        } else {
        ...
        if (new_group_session == false) {
            for (i = 0; i < msg->n_member_info_list; i++) {
                ...
                if (!compare_address(cur_group_member_id->member_address, receiver_address)) {
                    ret = new_inbound_group_session_by_member_id(
                        e2ees_pack_id, receiver_address, cur_group_member_id, group_info);
                    if (ret == E2EES_RESULT_FAIL) {
                        e2ees_notify_log(
                            receiver_address,
                            DEBUG_LOG,
                            "consume_remove_group_members_msg() "
                            "new_inbound_group_session_by_member_id failed "
                            "with member_address: [%s:%s]",
            ...
            for (i = 0; i < msg->n_member_info_list; i++) {
                ...
                if (!compare_address(cur_group_member_id->member_address, sender_address)) {
                    if (!compare_address(cur_group_member_id->member_address, receiver_address)) {
                        ret = new_and_complete_inbound_group_session(
                            cur_group_member_id, inbound_group_session);
                        if (ret == E2EES_RESULT_FAIL) {
                            e2ees_notify_log(
                                receiver_address,
                                DEBUG_LOG,
                    ...
                    ret = complete_inbound_group_session_by_member_id(
                        inbound_group_session, cur_group_member_id);
                    if (ret == E2EES_RESULT_FAIL) {
                        e2ees_notify_log(
                            receiver_address,
                            DEBUG_LOG,
            ...
            if (ret == E2EES_RESULT_FAIL) {
                e2ees_notify_log(
                    ...
                    DEBUG_LOG,
                    "consume_remove_group_members_msg() "
                    "new_outbound_group_session_by_receiver failed");
            }
        }
...
int produce_leave_group_request(
    E2ees__LeaveGroupRequest **request_out, leave_group_params_t *params) {
    int ret                           = E2EES_RESULT_SUCC;
...
int consume_leave_group_response(
    E2ees__E2eeAddress *user_address, E2ees__LeaveGroupResponse *response) {
    int ret = E2EES_RESULT_SUCC;
    ...
    if (ret == E2EES_RESULT_SUCC) {
        e2ees_notify_log(
            user_address,
            DEBUG_LOG,
            "consume_leave_group_response() success, unload group session");
        // unload
        get_e2ees_plugin()->db_handler.unload_group_session_by_address(
            user_address, response->group_address);
    } else {
        e2ees_notify_log(
            user_address, DEBUG_LOG, "consume_leave_group_response() failed, redo later");
    }
...
bool consume_leave_group_msg(E2ees__E2eeAddress *receiver_address, E2ees__LeaveGroupMsg *msg) {
    ...
    if (is_valid_address(receiver_address)) {
        if (is_valid_leave_group_msg(msg)) {
            ...
            e2ees_notify_log(
                NULL, BAD_LEAVE_GROUP_MSG, "consume_leave_group_msg() invalid leave_group_msg");
            ret = E2EES_RESULT_FAIL;
        }
    } else {
...
int produce_send_group_msg_request(
    ...
    if (params->allow_list_len > 0 && params->allow_list) {
        e2ees_notify_log(
            ...
            DEBUG_LOG,
            "produce_send_group_msg_request() with allow_list_len: %d",
            params->allow_list_len);
        request->n_allow_list = params->allow_list_len;
        request->allow_list =
            (E2ees__E2eeAddress **)malloc(sizeof(E2ees__E2eeAddress *) * params->allow_list_len);
    ...
    if (params->deny_list_len > 0 && params->deny_list) {
        e2ees_notify_log(
            params->sender_address,
            DEBUG_LOG,
            "produce_send_group_msg_request() with deny_list_len: %d",
            params->deny_list_len);
        request->n_deny_list = params->deny_list_len;
        request->deny_list =
            (E2ees__E2eeAddress **)malloc(sizeof(E2ees__E2eeAddress *) * params->deny_list_len);
...
int consume_send_group_msg_response(
    E2ees__GroupSession *outbound_group_session, E2ees__SendGroupMsgResponse *response) {
    int ret                      = E2EES_RESULT_SUCC;
...
bool consume_group_msg(E2ees__E2eeAddress *receiver_address, E2ees__E2eeMsg *e2ee_msg) {
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        get_e2ees_plugin()->db_handler.load_group_session_by_id(
            e2ee_msg->from, receiver_address, e2ee_msg->session_id, &inbound_group_session);

        if (!is_valid_group_session(inbound_group_session)) {
            e2ees_notify_log(
                receiver_address,
                BAD_GROUP_SESSION,
                "consume_group_msg() invalid inbound group session");
            ret = E2EES_RESULT_FAIL;
        }
    }
    ...
    if (ret == E2EES_RESULT_SUCC) {
        ...
        ret = cipher_suite->ds_suite->verify(
            ...
            group_msg_payload->signature.len,
        ...
        if (ret == E2EES_RESULT_SUCC) {
            ...
            if (ret == E2EES_RESULT_SUCC) {
                e2ees_notify_group_msg(
                    inbound_group_session->session_owner,
                    e2ee_msg->from,
                    inbound_group_session->group_info->group_address,
                    plaintext_data,
                    plaintext_data_len);
                ...
                e2ees_notify_log(
                    ...
                    "consume_group_msg(): decryption failed");
            }
        } else {
            e2ees_notify_log(
                inbound_group_session->session_owner,
                BAD_SIGNATURE,
                "consume_group_msg(): verification failed");
        }
    }
