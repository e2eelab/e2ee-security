...
static crypto_kem_param_t hqc128_param         = { true,
...
static crypto_kem_param_t hqc192_param         = { true,
...
static crypto_kem_param_t hqc256_param         = { true,
...
static crypto_kem_param_t mlkem512_param       = { true,
...
static crypto_kem_param_t mlkem768_param       = { true,
...
static crypto_kem_param_t mlkem1024_param      = { true,
...
static crypto_kem_param_t crypto_kem_params_hqc128() { return hqc128_param; }

static crypto_kem_param_t crypto_kem_params_hqc192() { return hqc192_param; }

static crypto_kem_param_t crypto_kem_params_hqc256() { return hqc256_param; }

static crypto_kem_param_t crypto_kem_params_mlkem512() { return mlkem512_param; }

static crypto_kem_param_t crypto_kem_params_mlkem768() { return mlkem768_param; }

static crypto_kem_param_t crypto_kem_params_mlkem1024() { return mlkem1024_param; }

static crypto_kem_param_t crypto_kem_params_mceliece348864() { return mceliece348864_param; }

static crypto_kem_param_t crypto_kem_params_mceliece348864f() { return mceliece348864f_param; }

static crypto_kem_param_t crypto_kem_params_mceliece460896() { return mceliece460896_param; }

static crypto_kem_param_t crypto_kem_params_mceliece460896f() { return mceliece460896f_param; }

static crypto_kem_param_t crypto_kem_params_mceliece6688128() { return mceliece6688128_param; }

static crypto_kem_param_t crypto_kem_params_mceliece6688128f() { return mceliece6688128f_param; }

static crypto_kem_param_t crypto_kem_params_mceliece6960119() { return mceliece6960119_param; }

static crypto_kem_param_t crypto_kem_params_mceliece6960119f() { return mceliece6960119f_param; }

static crypto_kem_param_t crypto_kem_params_mceliece8192128() { return mceliece8192128_param; }

static crypto_kem_param_t crypto_kem_params_mceliece8192128f() { return mceliece8192128f_param; }

static int
crypto_kem_asym_key_gen_hqc128(ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_HQC128_CLEAN_CRYPTO_SECRETKEYBYTES);
...
static int
crypto_kem_asym_key_gen_hqc192(ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_HQC192_CLEAN_CRYPTO_SECRETKEYBYTES);
...
static int
crypto_kem_asym_key_gen_hqc256(ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_HQC256_CLEAN_CRYPTO_SECRETKEYBYTES);
...
static int
crypto_kem_asym_key_gen_mlkem512(ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MLKEM512_CLEAN_CRYPTO_SECRETKEYBYTES);
    priv_key->len = PQCLEAN_MLKEM512_CLEAN_CRYPTO_SECRETKEYBYTES;

    pub_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MLKEM512_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
static int
crypto_kem_asym_key_gen_mlkem768(ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MLKEM768_CLEAN_CRYPTO_SECRETKEYBYTES);
    priv_key->len = PQCLEAN_MLKEM768_CLEAN_CRYPTO_SECRETKEYBYTES;

    pub_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MLKEM768_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
static int
crypto_kem_asym_key_gen_mlkem1024(ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MLKEM1024_CLEAN_CRYPTO_SECRETKEYBYTES);
    priv_key->len = PQCLEAN_MLKEM1024_CLEAN_CRYPTO_SECRETKEYBYTES;

    pub_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MLKEM1024_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
static int crypto_kem_asym_key_gen_mceliece348864(
    ...
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE348864_CLEAN_CRYPTO_SECRETKEYBYTES);
    priv_key->len = PQCLEAN_MCELIECE348864_CLEAN_CRYPTO_SECRETKEYBYTES;

    pub_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE348864_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
static int crypto_kem_asym_key_gen_mceliece348864f(
    ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE348864F_CLEAN_CRYPTO_SECRETKEYBYTES);
    priv_key->len = PQCLEAN_MCELIECE348864F_CLEAN_CRYPTO_SECRETKEYBYTES;

    pub_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE348864F_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
static int crypto_kem_asym_key_gen_mceliece460896(
    ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE460896_CLEAN_CRYPTO_SECRETKEYBYTES);
    priv_key->len = PQCLEAN_MCELIECE460896_CLEAN_CRYPTO_SECRETKEYBYTES;

    pub_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE460896_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
static int crypto_kem_asym_key_gen_mceliece460896f(
    ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE460896F_CLEAN_CRYPTO_SECRETKEYBYTES);
    priv_key->len = PQCLEAN_MCELIECE460896F_CLEAN_CRYPTO_SECRETKEYBYTES;

    pub_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE460896F_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
static int crypto_kem_asym_key_gen_mceliece6688128(
    ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE6688128_CLEAN_CRYPTO_SECRETKEYBYTES);
    priv_key->len = PQCLEAN_MCELIECE6688128_CLEAN_CRYPTO_SECRETKEYBYTES;

    pub_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE6688128_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
static int crypto_kem_asym_key_gen_mceliece6688128f(
    ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE6688128F_CLEAN_CRYPTO_SECRETKEYBYTES);
    priv_key->len = PQCLEAN_MCELIECE6688128F_CLEAN_CRYPTO_SECRETKEYBYTES;

    pub_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE6688128F_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
static int crypto_kem_asym_key_gen_mceliece6960119(
    ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE6960119_CLEAN_CRYPTO_SECRETKEYBYTES);
    priv_key->len = PQCLEAN_MCELIECE6960119_CLEAN_CRYPTO_SECRETKEYBYTES;

    pub_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE6960119_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
static int crypto_kem_asym_key_gen_mceliece6960119f(
    ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_SECRETKEYBYTES);
    priv_key->len = PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_SECRETKEYBYTES;

    pub_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
static int crypto_kem_asym_key_gen_mceliece8192128(
    ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE8192128_CLEAN_CRYPTO_SECRETKEYBYTES);
    priv_key->len = PQCLEAN_MCELIECE8192128_CLEAN_CRYPTO_SECRETKEYBYTES;

    pub_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE8192128_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
static int crypto_kem_asym_key_gen_mceliece8192128f(
    ProtobufCBinaryData *pub_key, ProtobufCBinaryData *priv_key) {
    priv_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE8192128F_CLEAN_CRYPTO_SECRETKEYBYTES);
    priv_key->len = PQCLEAN_MCELIECE8192128F_CLEAN_CRYPTO_SECRETKEYBYTES;

    pub_key->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE8192128F_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
static int crypto_kem_encaps_hqc128(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_HQC128_CLEAN_CRYPTO_CIPHERTEXTBYTES);
...
static int crypto_kem_encaps_hqc192(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_HQC192_CLEAN_CRYPTO_CIPHERTEXTBYTES);
...
static int crypto_kem_encaps_hqc256(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_HQC256_CLEAN_CRYPTO_CIPHERTEXTBYTES);
...
static int crypto_kem_encaps_mlkem512(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MLKEM512_CLEAN_CRYPTO_CIPHERTEXTBYTES);
...
static int crypto_kem_encaps_mlkem768(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MLKEM768_CLEAN_CRYPTO_CIPHERTEXTBYTES);
...
static int crypto_kem_encaps_mlkem1024(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MLKEM1024_CLEAN_CRYPTO_CIPHERTEXTBYTES);
...
static int crypto_kem_encaps_mceliece348864(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE348864_CLEAN_CRYPTO_CIPHERTEXTBYTES);
    ciphertext->len = PQCLEAN_MCELIECE348864_CLEAN_CRYPTO_CIPHERTEXTBYTES;
    return PQCLEAN_MCELIECE348864_CLEAN_crypto_kem_enc(
        ciphertext->data, shared_secret, their_key->data);
}

static int crypto_kem_encaps_mceliece348864f(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE348864F_CLEAN_CRYPTO_CIPHERTEXTBYTES);
    ciphertext->len = PQCLEAN_MCELIECE348864F_CLEAN_CRYPTO_CIPHERTEXTBYTES;
    return PQCLEAN_MCELIECE348864F_CLEAN_crypto_kem_enc(
        ciphertext->data, shared_secret, their_key->data);
}
...
static int crypto_kem_encaps_mceliece460896(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE460896_CLEAN_CRYPTO_CIPHERTEXTBYTES);
    ciphertext->len = PQCLEAN_MCELIECE460896_CLEAN_CRYPTO_CIPHERTEXTBYTES;
    return PQCLEAN_MCELIECE460896_CLEAN_crypto_kem_enc(
        ciphertext->data, shared_secret, their_key->data);
}
...
static int crypto_kem_encaps_mceliece460896f(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE460896F_CLEAN_CRYPTO_CIPHERTEXTBYTES);
    ciphertext->len = PQCLEAN_MCELIECE460896F_CLEAN_CRYPTO_CIPHERTEXTBYTES;
    return PQCLEAN_MCELIECE460896F_CLEAN_crypto_kem_enc(
        ciphertext->data, shared_secret, their_key->data);
}
...
static int crypto_kem_encaps_mceliece6688128(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE6688128_CLEAN_CRYPTO_CIPHERTEXTBYTES);
    ciphertext->len = PQCLEAN_MCELIECE6688128_CLEAN_CRYPTO_CIPHERTEXTBYTES;
    return PQCLEAN_MCELIECE6688128_CLEAN_crypto_kem_enc(
        ciphertext->data, shared_secret, their_key->data);
}
...
static int crypto_kem_encaps_mceliece6688128f(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE6688128F_CLEAN_CRYPTO_CIPHERTEXTBYTES);
    ciphertext->len = PQCLEAN_MCELIECE6688128F_CLEAN_CRYPTO_CIPHERTEXTBYTES;
    return PQCLEAN_MCELIECE6688128F_CLEAN_crypto_kem_enc(
        ciphertext->data, shared_secret, their_key->data);
}
...
static int crypto_kem_encaps_mceliece6960119(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE6960119_CLEAN_CRYPTO_CIPHERTEXTBYTES);
    ciphertext->len = PQCLEAN_MCELIECE6960119_CLEAN_CRYPTO_CIPHERTEXTBYTES;
    return PQCLEAN_MCELIECE6960119_CLEAN_crypto_kem_enc(
        ciphertext->data, shared_secret, their_key->data);
}
...
static int crypto_kem_encaps_mceliece6960119f(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_CIPHERTEXTBYTES);
    ciphertext->len = PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_CIPHERTEXTBYTES;
    return PQCLEAN_MCELIECE6960119F_CLEAN_crypto_kem_enc(
        ciphertext->data, shared_secret, their_key->data);
}
...
static int crypto_kem_encaps_mceliece8192128(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE8192128_CLEAN_CRYPTO_CIPHERTEXTBYTES);
...
static int crypto_kem_encaps_mceliece8192128f(
    uint8_t *shared_secret, ProtobufCBinaryData *ciphertext, const ProtobufCBinaryData *their_key) {
    ciphertext->data =
        (uint8_t *)malloc(sizeof(uint8_t) * PQCLEAN_MCELIECE8192128F_CLEAN_CRYPTO_CIPHERTEXTBYTES);
    ciphertext->len = PQCLEAN_MCELIECE8192128F_CLEAN_CRYPTO_CIPHERTEXTBYTES;
    return PQCLEAN_MCELIECE8192128F_CLEAN_crypto_kem_enc(
        ciphertext->data, shared_secret, their_key->data);
}
...
static int crypto_kem_decaps_mceliece348864(
    ...
    const ProtobufCBinaryData *our_key,
    const ProtobufCBinaryData *ciphertext) {
    return PQCLEAN_MCELIECE348864_CLEAN_crypto_kem_dec(
        shared_secret, ciphertext->data, our_key->data);
}
...
static int crypto_kem_decaps_mceliece348864f(
    ...
    const ProtobufCBinaryData *our_key,
    const ProtobufCBinaryData *ciphertext) {
    return PQCLEAN_MCELIECE348864F_CLEAN_crypto_kem_dec(
        shared_secret, ciphertext->data, our_key->data);
}
...
static int crypto_kem_decaps_mceliece460896(
    ...
    const ProtobufCBinaryData *our_key,
    const ProtobufCBinaryData *ciphertext) {
    return PQCLEAN_MCELIECE460896_CLEAN_crypto_kem_dec(
        shared_secret, ciphertext->data, our_key->data);
}
...
static int crypto_kem_decaps_mceliece460896f(
    ...
    const ProtobufCBinaryData *our_key,
    const ProtobufCBinaryData *ciphertext) {
    return PQCLEAN_MCELIECE460896F_CLEAN_crypto_kem_dec(
        shared_secret, ciphertext->data, our_key->data);
}
...
static int crypto_kem_decaps_mceliece6688128(
    ...
    const ProtobufCBinaryData *our_key,
    const ProtobufCBinaryData *ciphertext) {
    return PQCLEAN_MCELIECE6688128_CLEAN_crypto_kem_dec(
        shared_secret, ciphertext->data, our_key->data);
}
...
static int crypto_kem_decaps_mceliece6688128f(
    ...
    const ProtobufCBinaryData *our_key,
    const ProtobufCBinaryData *ciphertext) {
    return PQCLEAN_MCELIECE6688128F_CLEAN_crypto_kem_dec(
        shared_secret, ciphertext->data, our_key->data);
}
...
static int crypto_kem_decaps_mceliece6960119(
    ...
    const ProtobufCBinaryData *our_key,
    const ProtobufCBinaryData *ciphertext) {
    return PQCLEAN_MCELIECE6960119_CLEAN_crypto_kem_dec(
        shared_secret, ciphertext->data, our_key->data);
}
...
static int crypto_kem_decaps_mceliece6960119f(
    ...
    const ProtobufCBinaryData *our_key,
    const ProtobufCBinaryData *ciphertext) {
    return PQCLEAN_MCELIECE6960119F_CLEAN_crypto_kem_dec(
        shared_secret, ciphertext->data, our_key->data);
}
